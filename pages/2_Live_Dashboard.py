"""
Solar Sense — Live Dashboard (real-time technical demo).
KPIs, time-series, IV curve, raw telemetry; simulated and serial modes.
"""
import time
import streamlit as st
import pandas as pd
import plotly.graph_objects as go
# Local data layer (no Streamlit inside)
...
if "events" not in st.session_state:
    st.session_state.events = []  # list of (timestamp_ms, label)
...
with st.sidebar:
    ...
    if st.button("Mark event"):
        # Use latest timestamp from last row if available, else current time in ms
...
if st.session_state.data_source == "Simulated":
    ...
    if row is None:
        # No serial data: keep last state, do not crash
        row = None
...
# ---------------------------------------------------------------------------
# Auto-refresh: rerun after refresh_rate seconds
# ---------------------------------------------------------------------------
